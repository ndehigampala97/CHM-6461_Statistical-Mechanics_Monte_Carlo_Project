...
def count_contacts(coords: Coords, occupied: Occupied) -> int:
...
def propose_crankshaft(coords: Coords, occupied: Occupied) -> Optional[Tuple[Coords, Occupied]]:
    ...
    if not (is_neighbor(a, new_b) and

            is_neighbor(new_b, new_c) and
...
def run_mc_saw(
    ...
    csv_path: str = "saw_observables.csv",
    ...
    for step in range(1, n_steps + 1):
        ...
        # write trajectory frames + record observables
        ...
        if step % sample_every == 0:
            ...
            R_list.append(end_to_end_distance(coords))

            Rg_list.append(radius_of_gyration(coords))

            C_list.append(count_contacts(coords, occupied))
        ...
        if step % print_every == 0:
            ...
            acc_given_valid = accepted / valid_proposals if valid_proposals else 0.0  # 1.0 for SAW

            c_now = count_contacts(coords, occupied)

            print(

                f"step {step:>6} | valid {valid_frac:>6.3f} | acc(valid) {acc_given_valid:>6.3f} "

                f"| R {end_to_end_distance(coords):.3f} | Rg {radius_of_gyration(coords):.3f} | C {c_now}"
    ...
    with open(csv_path, "w", newline="") as f:
        ...
        w.writerow(["step", "R", "Rg", "contacts"])

        for s, R, Rg, c in zip(steps_list, R_list, Rg_list, C_list):

            w.writerow([s, R, Rg, c])



    # Averages after burn-in (discard first 20% samples)

    if len(R_list) > 0:

        burn = int(0.2 * len(R_list))

        burn = min(burn, len(R_list) - 1)  # safety



        R_avg = sum(R_list[burn:]) / len(R_list[burn:])

        Rg_avg = sum(Rg_list[burn:]) / len(Rg_list[burn:])

        C_avg = sum(C_list[burn:]) / len(C_list[burn:])



        print("\nSAW averages (after burn-in):")
        ...
        print(f"Burn-in removed: {burn} of {len(R_list)} samples")
    ...
    print(f"Observables written to: {csv_path}")
