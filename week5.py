...
def initial_straight_chain(N):
    ...
    occ = {pos:i for i,pos in enumerate(coords)}
...
def write_xyz_frame(fh, coords, frame):
...
def propose_end_move(coords, occ):
    ...
    cands = [p for p in neighbors(anchor) if p != old_end and p not in occ]
    ...
    new_occ[new_end] = end_idx

    return new_coords, new_occ



def run(N=6, steps=2000, sample_every=50, seed=0, traj="traj.xyz"):
    ...
    with open(traj, "w") as fh:
        ...
        for step in range(1, steps+1):
            ...
            prop = propose_end_move(coords, occ)
            ...
            if step % sample_every == 0:
                ...
                write_xyz_frame(fh, recenter(coords), step)
